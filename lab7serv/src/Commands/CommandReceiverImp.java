...
public class CommandReceiverImp implements CommandReceiver {
    ...
    public boolean checkUser(String login, String password, Socket socket) throws DatabaseException, IOException {
        ...
        if (exist) {
            logger.info(String.format("Пользователь %s, адрес %s:%s - подключился", login, socket.getInetAddress(), socket.getPort()));
            return true;
        } else {
            sendObject(socket, new SerializedMessage("Пользователь не подключен!"));
            logger.info(String.format("Пользователь %s:%s не подключен.", socket.getInetAddress(), socket.getPort()));
        }
